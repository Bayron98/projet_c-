...
void Vol::fill(){
    ...
    cout << "Entrez le nombre des passagers: "; cin >>_nbr_passagers;
    ...
    do{
        ...
        cin >> date;
        _date_depart = time_from_string(date);
        i++;
    }while(!check_date_time(date));
    cout << "Entrez la date d'arrivée (YYYY-MM-DD HH:MM): ";
    string date2;
    int j = 0;
    do{
        if (j != 0){cout << "Date invalide"<<endl;}
        cin >> date2;
        _date_arrivee = time_from_string(date2);
        j++;
    }while(!check_date_time(date2));
